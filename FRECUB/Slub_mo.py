...
class SCLUB(LinUCB_IND):
    ...
    def store_info(self, i, x, y, t, r=1, br=1):
    ...
    def split_fd(self, i, t):
        ...
        if self._split_or_merge_p(self.N[i] / (t + 1), self._cluster_avg_freq(c, t + 1), t + 1,
            ...
            # cluster.S = cluster.S - fre[ind] * self.S[i]
        ...
        for c1 in range(cmax + 1):
            ...
            for c2 in range(c1 + 1, cmax + 1):
                ...
                if self._split_or_merge(self.clusters[c1].theta - self.clusters[c2].theta, self.clusters[c1].N,
                    ...
                    # self.clusters[c1].S = np.zeros((self.d, self.d))
                    self.clusters[c1].b = np.zeros(self.d)
                    for k in range(len(self.clusters[c1].users)):
                        # self.clusters[c1].S += fre[k] * self.S[self.clusters[c1].users[k]]
    ...
    def get_items(self, user):
        ...
        for key, value in self.user_items.items():
            name = key.split("_")
            if str(user) == name[0] and value >= 1:
                mid.append(int(name[1]))
        sample_num = 24
        pool_id = random.sample(self.items, sample_num)
        dif = list(set(mid) - set(pool_id))
        if dif == []:
    ...
    # def feedback(self, i, k, theta):
    #
    #     if
    #     x = self.items[k, :]
    #     r = np.dot(self.theta[i], x)
    #     y = np.random.binomial(1, r)
    #     br = np.max(np.dot(self.items, theta[i]))
    #     return y, r, br
            ...
            for t in range(2 ** s):
                ...
                for i in I:
                    ...
                    if self.items[items_id[kk]] in mid:
                        ...
                        reward = 0
                        r = 0
                    self.store_info(i, x, reward, tau, r, br=1)
